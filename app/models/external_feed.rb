  ...
  def add_entry(item, feed, feed_type)
    if feed_type == :rss
      ...
      if uuid && uuid.length > 255
        uuid = Digest::SHA384.hexdigest(uuid)
      end
      uuid ||= Digest::SHA384.hexdigest("#{item.title}#{item.date.strftime('%Y-%m-%d')}")
      ...
      uuid = item.id || Digest::SHA384.hexdigest("#{item.title}#{item.published.utc.strftime('%Y-%m-%d')}")
      entry = self.external_feed_entries.where(uuid: uuid).first
